/**
 * FounderOS — OpenCode Automated PR Review Engine
 * ====================================================
 * Invokes OpenCode CLI in headless mode (`opencode run`) to conduct adversarial
...
import { findOpenCodeBinary, buildExecutorEnv } from "../src/tools/opencode.js";
...
async function reviewPR(prNumber: string, binary: string, instructions: string): Promise<void> {
  console.log(`\n================================================================================`);
  console.log(`🤖 Triggering OpenCode Adversarial Review & Self-Fixing on PR #${prNumber}...`);
  console.log(`================================================================================\n`);
...
`;
  ...
  await new Promise<void>((resolve) => {
    ...
    child.on("close", (code) => {
      if (code === 0) {
        console.log(`\n✅ OpenCode review process for PR #${prNumber} completed.`);
      } else {
        console.error(`\n❌ OpenCode review process for PR #${prNumber} exited with code ${code}.`);
      }
      resolve();
    });
  });
...
async function main(): Promise<void> {
  ...
  if (!binary) {
    console.error("❌ OpenCode CLI binary not found.");
    process.exit(1);
  }
