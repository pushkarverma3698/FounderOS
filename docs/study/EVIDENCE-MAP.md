...
| **Scalability** | 45.2% | 🟡 | 3,223 ATS boards polled, 890 ingest runs/month, single-VPS. Horizontal scale is **not** demonstrated |
...
| **Testing** | 39.5% | ✅ | **4,433 tests across 397 files, $0 per run** — scripted models let the full graph run in CI without paid calls |
...
| demand | % | status | proof |
|---|---|---|---|
| **CI/CD** | 32.8% | ✅ | `pnpm gate` = lint + build + wiring + arch + 4,433 tests; branch → beta → main with CD to prod |
...
> ratcheted in CI, and the whole graph runs offline in 4,433 tests at $0 — with 229 real
