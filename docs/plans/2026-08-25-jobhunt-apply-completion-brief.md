...
### T1a — a failed CV fetch is swallowed
...
## T5 · Put the cover letter where his hands are
...
- **No production database access** from the authoring session. The claim "almost no row carries a
  tailored CV" is argued from the call graph, not counted. **The query at the top of "start applying"
  settles it in one command — run that first.**
- **`$STORAGE_BUCKET` over non-interactive SSH** — unchecked. Named in T1a.
