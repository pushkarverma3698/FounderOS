...
export const readFile = registerTool(
  tool(
    async ({ path: filePath }) => {
  ...
  ),
  {
    ...
    hitlGated: false,
    promptDescription: "read a text file and show its CONTENTS as text in the chat. Read-only, instant, no approval.",
  }
);

// ── Personal: list a directory (read-only, NO approval) ───────────────────────

export const listDir = registerTool(
  tool(
    async ({ path: dirPath }) => {
  ...
  ),
  {
    departments: ["personal"],
    hitlGated: false,
    promptDescription: "list a directory's contents. Read-only, instant, no approval.",
  }
);

// ── Personal: send a laptop file to Telegram (OUTBOUND — requires approval) ───

export const sendFile = registerTool(
  tool(
    async ({ path: filePath }) => {
  ...
  ),
  {
    departments: ["personal"],
    hitlGated: true,
    promptDescription: "ATTACH a file from his laptop and deliver it INTO this Telegram chat as a downloadable document (any file type — PDF, image, zip, code). The founder must APPROVE before it sends.",
  }
);

// ── Personal: write a file (WRITE — requires approval) ────────────────────────

export const writeFile = registerTool(
  tool(
  ...
  ),
  {
    departments: ["personal"],
    hitlGated: true,
    promptDescription: "create/overwrite a file. The founder must APPROVE before it writes.",
  }
);

// ── Personal: run a shell command / script (WRITE — requires approval) ─────────

export const runShell = registerTool(
  tool(
    async ({ command, cwd }) => {
  ...
  ),
  {
    departments: ["personal"],
    hitlGated: true,
    promptDescription: "run a shell command/script (cwd confined to his personal root). The founder must APPROVE before it runs.",
  }
);

// ── Personal: drive Safari via AppleScript (WRITE — requires approval) ─────────

export const browser = registerTool(
  tool(
    async ({ action, url, js }) => {
      const rejected = hitlGate({
        action: "browser",
  ...
  ),
  {
    departments: ["personal"],
    hitlGated: true,
    promptDescription: "drive Safari: open_url, get_page_text, run_js. The founder must APPROVE before it runs.",
    notes: "Safari automation on the founder's Mac (personal dept).",
  }
);
