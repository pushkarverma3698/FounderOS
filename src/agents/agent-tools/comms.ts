...
export const sendEmail = registerTool(
  tool(
  ...
  ),
  {
    ...
    hitlGated: true,
    promptDescription: "send an email (requires founder approval before sending)",
  }
);

// ── Marketing: LinkedIn post (WRITE — requires approval) ──────────────────────

export const linkedinPost = registerTool(
  tool(
    async ({ text }, config) => {
      // Brand voice check with a deterministic retry cap — runs before interrupt()
      // so the HITL card only shows clean content. Within the cap the agent
  ...
  ),
  {
    ...
    hitlGated: true,
    promptDescription: "publish a post to LinkedIn (requires founder approval)",
  }
);

// ── Comms: Google Calendar (WRITE — requires approval) ────────────────────────

export const createCalendarEvent = registerTool(
  tool(
  ...
  ),
  {
    departments: ["comms"],
    hitlGated: true,
    promptDescription: "add an event or reminder to Google Calendar (requires founder approval)",
  }
);

// ── Comms: read emails (read-only, NO approval) ────────────────────────────────

export const readEmails = registerTool(
  tool(
    async ({ query, limit }) => {
  ...
  ),
  {
    departments: ["comms"],
    hitlGated: false,
    promptDescription: "read Gmail inbox (read-only, no approval). Gmail syntax: \"is:unread\", \"from:alice@example.com\", \"subject:invoice\".",
  }
);
