...
export function buildCapabilityManifest(): string {
  ...
  return [
    ...
    "- FounderOS also RUNS a read-only MCP server exposing search_web, read_context, search_knowledge, search_memory, read_cv, github_read to external MCP clients — via stdio (pnpm mcp) or bearer-gated Streamable HTTP bound to 127.0.0.1:3100 (pnpm mcp:http), reached from the Mac over an SSH tunnel.",
  ].join("\n");
}
