...
function renderVisionStage(stage: VisionStage): string[] {
  ...
  for (const v of verdicts) {
    ...
    lines.push(`### ${status} — \`${v.target}\` @ ${v.viewport}`, "");
    if (!v.readable) {
...
export function renderEvidencePack(
  ...
  const lines: string[] = [
    ...
    pass
      ...
      : `❌ **FAIL** — ${highSeverityCount} blocking defect(s) across ${failingRows} of ${rows.length} page/viewport combination(s).`,
    "",
    ...
    "---",
    "_Generated by `pnpm qa:ui` — see `docs/antigravity/UI-QA-CONTRACT.md`._",
  ];
