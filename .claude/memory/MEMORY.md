...
- **Verified:** tsc clean · 1468 unit green · flag-off no-op · live probe loaded 13 tools from `server-everything` over real stdio, `echo` correctly gated. MTProto gate→approve→audit row = remaining PR-ready manual QA.
- **Wiring Map 5** added to PROGRAMMING-RULES (add a server = 1 JSON entry + restart, no code).
